...
def decode_mail_header(value, default_charset="us-ascii"):
    ...
    try:
        ...
        for index, (text, charset) in enumerate(headers):
            try:
                decoded_data = str_decode(text, charset or "utf-8", "replace")
                logger.debug(
                    f"Mail header no. {index}: {decoded_data} encoding {charset}",
                )
...
def get_mail_addresses(message, header_name):
    ...
    for index, (address_name, address_email) in enumerate(addresses):
        ...
        logger.debug(f"{header_name.upper()} Mail address in message: <{address_name}> {address_email}")
    return addresses
...
def parse_attachment(message_part):
    ...
    if content_disposition is not None and not message_part.is_multipart():
        dispositions = [
            disposition.strip() for disposition in parse_content_disposition(content_disposition) if disposition.strip()
        ]
...
def parse_email(raw_email, policy=None):
    ...
    if maintype in ("multipart", "image"):
        ...
        for part in email_message.walk():
            ...
            is_inline = content_disposition is None or content_disposition.startswith("inline")
